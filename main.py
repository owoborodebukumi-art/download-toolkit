...
def setup_android():
    ...
    if not os.environ.get('TMUX'):
        if shutil.which('tmux'):
            ...
            try:
                os.execvp('tmux', ['tmux', 'new-session', '-A', '-s', 'download', sys.executable] + sys.argv)
            except Exception as e:
...
def resolve_relative_url(base_url, location):
    ...
    return bd + location if bd else None
    ...
    try:
        ...
        if result.returncode == 0:
            if os.path.exists(filepath):
                ...
                if size < 1024 * 100:
                    print(f"  [✗] File too small ({size_mb:.2f}MB) — likely an error page")
                    try:
                        os.remove(filepath)
                    except Exception:
                        pass
...
def download_with_ytdlp(url, folder, filename, summary):
    ...
    if not HAS_YTDLP:
        if not install_ytdlp():
            ...
            return False
    if not HAS_FFMPEG:
        print(f"  [!] ffmpeg not found — install with: pkg install ffmpeg")
        summary.failed += 1
        return False
    os.makedirs(folder, exist_ok=True)
    base = re.sub(r'\.(mp4|mkv|m3u8)$', '', filename)
...
def resolve_vikingfile(url, session):
    """
    Resolve vikingfile.com to direct CDN link with md5 token.
...
def extract_naijavault(url, session):
    ...
    # Find all /dl- episode links on series page
    ...
    # Handle single movie/dl page
    if not dl_links:
        vf = re.search(r'var downloadURL = "([^"]+)"', r.text)
        if vf:
            dl_links = [(slug, url)]
    ...
    for i, (label, dl_url) in enumerate(dl_links, 1):
        ...
        print(f"\n[{i}/{len(dl_links)}] {ep_name}")

        session.headers.update({'Referer': url})
        r2 = safe_get(session, dl_url, timeout=20)
        if not r2:
            print(f"  [✗] Could not fetch download page")
            summary.failed += 1
            continue

        # Extract vikingfile URL from JS variable
        ...
        if vf_match:
            ...
            if direct:
                ...
                print(f"  [✗] VikingFile resolution failed")
                summary.failed += 1
            time.sleep(1)
            continue

        # Secondary host: cdn.filevault.com.ng (direct link)
        ...
        print(f"  [✗] No download URL found on page")
        summary.failed += 1
        time.sleep(1)
